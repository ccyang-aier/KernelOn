...
class WallpaperSourceModel(OrmBase):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "provider", name="uq_wallpaper_source_org_provider"),)
...
class WallpaperAssetModel(OrmBase):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "provider", "external_id", name="uq_wallpaper_asset_org_external"),
...
class WallpaperFavoriteModel(OrmBase):
    ...
    asset_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("wallpaper_assets.id", ondelete="CASCADE"), primary_key=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class WallpaperAssignmentModel(OrmBase):
    ...
    user_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), primary_key=True)
    organization_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("organizations.id", ondelete="CASCADE"), index=True)
    asset_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("wallpaper_assets.id", ondelete="RESTRICT"))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
