...
from uuid import UUID  # noqa: TC003 - Litestar resolves parameter annotations at runtime

from litestar import Controller, Request, Response, delete, get, patch, post, put
from litestar.di import NamedDependency  # noqa: TC002 - Litestar runtime dependency marker
...
class WallpaperController(Controller):
    ...
    async def search(
        ...
        wallpaper_service: NamedDependency[WallpaperService],
        q: str = "",
        media_type: str = "all",
        page: int = 1,
        limit: int = 24,
    ) -> dict[str, Any]:
        return await wallpaper_service.search(
            await _user(request, identity_service), q, media_type, page, limit
        )
    ...
    @post("/wallpapers/{asset_id:str}/import", operation_id="wallpaper_import")
    async def import_asset(
        ...
        return await wallpaper_service.import_asset(
            await _user(request, identity_service), asset_id, data.confirm
        )
    ...
    async def update_source(
        ...
        return await wallpaper_service.update_source(
            await _user(request, identity_service), source_id, data.enabled
        )

    @patch(
        "/me/wallpaper-source-preferences/{source_id:str}",
        operation_id="wallpaper_source_preference_update",
    )
    async def preference(
        ...
        return await wallpaper_service.set_source_preference(
            await _user(request, identity_service), source_id, data.visible
        )
    ...
    async def favorite(
        ...
        return await wallpaper_service.favorite(
            await _user(request, identity_service), data.asset, True
        )

    @delete(
        "/wallpapers/{asset_id:str}/favorite",
        operation_id="wallpaper_favorite_delete",
        status_code=200,
    )
    async def unfavorite(
        ...
        data.asset["id"] = asset_id
        return await wallpaper_service.favorite(
            await _user(request, identity_service), data.asset, False
        )
    ...
    async def create_upload(
        ...
        return await wallpaper_service.create_upload(
            await _user(request, identity_service), data.model_dump(by_alias=True)
        )
    ...
    async def upload_content(
        ...
        return await wallpaper_service.write_upload(
            await _user(request, identity_service), upload_id, await request.body()
        )

    @delete(
        "/wallpaper-uploads/{upload_id:uuid}",
        operation_id="wallpaper_upload_delete",
        status_code=204,
    )
    ...
    async def media(
        ...
        body, content_type = await wallpaper_service.media(
            await _user(request, identity_service), asset_id
        )
