...
WindowShell 不负责 App 的侧边栏宽度、标题对齐目标、内容分栏或业务工具组。
...
- 消费 WindowShell 提供的安全区，但不把 App 布局状态反向交给 Desktop Shell 计算。
...
| 完全组合 | App 自行组合 Header 区域和视觉，WindowShell 只保留窗口控制安全区 | 壁纸、画布、编辑器和沉浸式 App |
...
灵活性的最终逃生口是“完全组合”，而不是绕过 WindowShell 或操作其内部 DOM。
...
- `packages/shell`：Desktop Shell、WindowShell、窗口运行时和与窗口能力紧密相关的 AppFrame 基础设施。
...
- `AppHeaderSlot` 通过 Effect 将 ReactNode 注册到父容器，形成二阶段渲染和反向数据流。
- `AppWindowMount` 通过具体 App ID 判断特殊窗口层，Shell 因而认识业务 App。
- App 通过 Shell 内部 DOM、测试标识或固定像素修正 Header 几何。
- `standard`、`composable`、`immersive` 只有声明差异，尚未形成完整布局能力。

## 演进约束

- 不允许业务 App 自行复制 WindowShell、三色按钮或窗口交互。
...
最终目标是：**所有 App 共享同一个 WindowShell 和 AppFrame 契约，但任何 App 都能在契约内自由组合自己的 Header、Sidebar、内容结构和视觉语言。**
