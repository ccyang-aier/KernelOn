...
Wallpaper 内所有同类 action、顶栏容器、导航胶囊、轮播箭头和卡片预览标签统一附加 `wallpaper-frosted-surface`。该 class 只负责材质，并通过 `--wallpaper-frosted-*` token 提供边框、填充、阴影与模糊；各业务 class 只负责尺寸、布局和交互。

当前边框使用 `rgba(218, 238, 255, 0.30)`：相比纯白边框更偏冷调、不会显得发浅，同时在复杂图像背景上比原先 `0.16` 更清晰。hover 使用 `rgba(231, 246, 255, 0.42)`。卡片标签将 `--wallpaper-frosted-backdrop` 置为 `none`，只在 hover/focus 时恢复 `blur(2px) saturate(1.02)`，避免长列表的持续合成开销。
