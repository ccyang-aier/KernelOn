...
> **2026-07-11 实施状态补充**：本文关于两套 Liquid Glass 引擎的判断保留为架构研究结论，但 Wallpaper 已删除全部业务运行时接入，四个按钮统一回退到 CSS 清透磨砂玻璃。Ybouane 方案的最终可运行示例与经验归档在 `packages/ui/src/components/liquidglass/examples/`；未来重新启用真实折射前，必须先在隔离 Demo 完成视觉、动态背景和性能门禁。
