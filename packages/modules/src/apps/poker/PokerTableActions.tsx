...
export function PokerTableActions({
  ...
  return (
    <footer className="col-span-2 grid min-h-0 grid-cols-[31.7%_30.4%_37.9%] bg-[linear-gradient(180deg,#141718,#0e1112)]">
      <section className="border-r border-[#343635] px-[18px] py-4">
        <form
        ...
        <div className="mt-4 grid grid-cols-[1fr_auto] items-center gap-5 border-t border-white/[.07] pt-3">
          ...
          >
            ...
            <span className="text-[16px] font-semibold tabular-nums">12,450</span>
            <ChevronDown className="size-3.5" />
          </button>
        </div>
      </section>

      <section className="border-r border-[#343635] px-5 py-4">
        ...
        <div className="mt-4 px-1">
          <div className="relative h-2 rounded-full bg-[#333735]">
            ...
            <input
              aria-label="加注金额"
              className="absolute inset-0 z-10 h-7 w-full -translate-y-[9px] cursor-pointer opacity-0"
              max={8640}
              min={20}
        ...
        <div className="mt-3 grid grid-cols-4 gap-3">
          <PresetButton label="1 / 2" onClick={() => onChoosePreset('half')} />
          <PresetButton label="2 / 3" onClick={() => onChoosePreset('twoThirds')} />
          <PresetButton label="底池" onClick={() => onChoosePreset('pot')} />
          <PresetButton label="全下" onClick={() => onChoosePreset('allIn')} />
        </div>
      </section>

      <section className="grid grid-cols-[.92fr_.9fr_1.28fr] gap-3 px-3.5 py-[44px]">
        <ActionButton label="弃牌" onClick={() => onAction('fold')} tone="neutral" />
        <ActionButton label="跟注" onClick={() => onAction('call')} sublabel="240" tone="gold" />
        <ActionButton
          label="加注至"
...
function PresetButton({ label, onClick }: Readonly<{ label: string; onClick(): void }>) {
  return (
    <button
      className="h-10 rounded-md border border-[#65533a]/70 bg-[linear-gradient(145deg,#202222,#191b1b)] text-[13px] text-[#cac8c0] shadow-[inset_0_1px_0_rgba(255,255,255,.035)] transition duration-200 hover:-translate-y-0.5 hover:border-[#bb9556]/75 hover:bg-[#28251f] hover:text-[#ead4a7] active:translate-y-0"
      onClick={onClick}
      type="button"
    >
      {label}
