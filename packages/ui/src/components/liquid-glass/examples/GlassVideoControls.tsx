 ...
 * WebGL renderer that reads the playing frame and draws every lens from it â€?pass
...
// buttons above. A thin bar wants a GENTLE, flat refraction â€?the buttons'
...
// 104px; the skips are 0.6Ã— of it, offset to either side of centre.
const PLAY_DIA = 104;
const SKIP_DIA = 62;
...
// rounds the ends into a lozenge/oval â€?keep the corner small on a wide thin bar)
...
/** A circular-arrow skip glyph (~300Â° ring + a triangular pointer) wrapping the
...
const ScrubBar: React.FC<{
  ...
  // No background â€?a glass lens behind this (see SCRUB_* lens) IS the bar, so the
...
export const GlassVideoControls: React.FC<{
  ...
  /** Override the scrub-track optics (merged over {@link SCRUB_OPTICS}) â€?the
  ...
  // Drive the timeline fill from the real playback position (imperative â€?no
