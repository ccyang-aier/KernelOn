...
import { Glass, type GlassOptics } from "..";
...
// overlay (see boxShadow below), not the lens edge â€?which keeps it clean.
...
// No flat CSS fill â€?the bright frosted glass (lens brightness veil) is the
...
// A realistic macOS Finder context menu â€?native menus aren't red, and the
// right-click menu doesn't show âŒ?shortcuts (those live in the menu-bar menus).
...
export const GlassContextMenu: React.FC<GlassContextMenuProps> = ({
  ...
  const onContext = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Clamp the menu inside the container. `Math.max(8, â€?` on the upper bound
    ...
    >
      ...
      {ready && (
        <>
          ...
          />
          {/* Rows + the panel edge â€?crisp, on top of the glass. Same box as the
