...
// Motion signature â€?distinct overshoot + ease-out timings (same as the switch).
...
export const SLIDER_BASE: Partial<GlassOptics> = {
  ...
  // depth â‰?0.12 is a ~1.3â€?px refractive rim; curvature â‰?0.30 shapes the rim BEND
  ...
  // through a glassy-rimmed thumb â€?NOT a magnifying body dome. Per-axis scaleX/scaleY
  // (vs the switch's single `strength`): a wide thumb over a thin horizontal track
  // wants more vertical than horizontal bend (Safari more still â€?see SLIDER_SAFARI).
  depth: 0.2,
  dispersion: 0.5,
  scaleX: 0.06,
  scaleY: 0.06,
  ...
  // blooms) is themed per scheme â€?see SLIDER_DARK / SLIDER_LIGHT below.
};
...
export interface GlassSliderProps {
  ...
  /** Height of the refraction copy of the track (default 0.75 Ã— thumbHeight). */
  ...
  /** Resolved colour scheme â€?picks the light/dark lens preset. */
  scheme?: "light" | "dark";
...
export const GlassSlider: React.FC<GlassSliderProps> = ({
  ...
  // Refraction-band height: 0.75 Ã— thumbH.
